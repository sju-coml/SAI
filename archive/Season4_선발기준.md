<br>

### PC 로 확인 부탁드립니다.

<br>

# Season4 점수기준

- 많은 분들이 열심히 신청서를 작성해 주셨지만, 저희가 생각하던 인원 이상으로 너무 많은 분들이 관심을 가져주셔서 불가피하게 선발기준을 두게 되었습니다.
- 2020년 지원자 (60명) 의 두 배인 120명이 지원해 주셨습니다.
- 저희 스스로도 "체계가 잡히지 않은" 동아리라고 생각을 했지만, 저희 기준에 정말 많은 분들이 지원해 주신만큼 Season4 는 조금 더 부담감을 가지게 되는 것 같습니다.
- 지원자가 많아지면서 모든 지원자마다 4명이 검토하지 못하고, 최소 3인이 검토한 후, 3인의 점수 중 최저점수와 최고점수를 평균으로 보간한 뒤 더해서 산출했습니다.


<br>

| 동아리 지속성, 성실성 기대 (동아리 적합성, 열정) | 타당성 + 개연성 + 실력 (개연성 있는 성장, 그동안 걸어온 길) | 다른단과대 | 학년 |
|:-------------------------------:|:-------------------------------------------:|:----------:|:----:|
| Season 1\~3 운영진 2\~3명,<br>인당 0-4점 부여 | Season 1\~3 운영진 2\~3명,<br> 인당 1~3점 부여| 타 단대 or<br>창의소프트 | 돌아오는 학기<br>2,3학년|
| 각 2점, 대부분 문항 무응답시 0점 | 학과, 걸어온 길 바탕으로 상대적으로 평가 | 2점 가산점 | 1점 가산점 |

<br><br>

# 인원

<br>

- google spreadsheet 로 설문을 옮겨 이름 colunm 을 삭제한 60명의 지원서를 4번씩 읽고 31명에게 긍정적 답변을 드렸습니다
- 불합격 안내를 드린 ? 명 중 ? 명이 추가적으로 지원했습니다.

<br><br>

# 평가상세 및 취지

<br>

- 2019년 초, 인공지능이라는 것 자체를 2학년 1학기의 몸으로 배우기 어려웠고, 접하기 어려웠던 제 (장후) 의 고통이 있었던 만큼, 최대한 많은 분께 접근성을 높여 주고 싶은 꿈이 있습니다.
- 못하는 건 전 혀 상관 없습니다.
- 하지만, 다양한 환경 속에서도 자신이 해볼 수 있는 최소한의 시도를 해 본 분들이라고 생각이 들 수 없었던 분들은, 저희는 '능동적으로' '떠먹여주지 않아도 움직이는' 사람이라고 확신할 수 없었습니다.
- e.g. 1학년 1학기인데 고등학교에서 이런 활동을 했지만 저런 한계에 봉착하여, 여기에 지원을 했다는 경우
- e.g. 1학기인데 정말 많은 분야를 돌고 돌았지만 결국 여기가 관심사인 것 같아서, 여기에 지원을 했다는 경우
- 좋지 않은 예시로, SW 계열 2~3학년 1학기임에도 불구하고, "이제 해보려고 한다" 또는 "관심이 있었지만" "최소한의 공부를 해보려는 액션" 조차 없었던 분들에게 저는 좋지 않은 평가를 드릴 수밖에 없었습니다.
- 저희 운영진들은 항상 노력하지만, 직업이 아니기 때문에, 여러분들과 똑같이 모든 일을 할 수 없습니다. SAI 는 교육기관이 아닙니다. 스스로 하고 싶은 분야에 대해서 열정이 있어서, 자율적으로 스터디를 꾸리고 정해진 시간에 스스로 나오는 분들을 뽑아야 동아리가 돌아간다는 사실을 2020년 검증했기 때문에, 이런 기준을 두게 되었고 선발을 해서 운영하게 되었습니다. 
- 아무리 실제로 그런 생각을 가지고 계신 분들이어도, 정말 안타깝게도 그러한 모습이 지원서에 드러나지 않았을 경우 좋은 평가를 드리기 어려웠다는 점 양해 부탁드립니다.
- 비슷한 맥락에서, 다양한 도메인 (인문, 예술, 공학) 이 드러나게 서술해주신 분께 좋은 평가를 드렸습니다. 다른 분야에서, 이 분야에 관심을 가지기 위한 장벽이 높다고 판단했기 때문입니다.
- 더 자세히, 길게 작성해 주신 분이 "상대적으로 더 오래할 수 있고 더 열정이 있댜" 라고 절대적으로 판단하지는 않지만, 100자도 안되는 내용을 바탕으로 작성해주신 분들이 많았습니다. 
- 이런 경우, 어떤 분인지 파악하는 위험성이 더 높다고 생각해서, 내용이 짧은 분들은 상대적으로 점수가 낮을 수밖에 없었습니다.
- S팀의 경우 안내했던 대로 조기선발했습니다.
- 우선 긴 문항들로 괴롭혀서 죄송하고, 서류 따위로 점수를 매겨서 정말 죄송합니다.
- 이 절차는 '관심도 열정도 시간도 별로 없는데 그냥 한 번 지원해 봤다' 라고 생각하여 분들을 최대한 줄이기 위한 작업이기에, 깊은 이해를 부탁드립니다.

<br>

### 아래는 지원서에 대해 위 피드백을 적용한 예시입니다.

<br>

> 고등학교때 고등학교 생활기록부 채우기 위해서 마스터 알고리즘이라는 책을 읽었는데, 거기서 설명하는 딥러닝이 인공지능과 관련이 있는것 같아서 인공지능에 관심을 가지게 되었다. 인간이 배우는 방식을 연구하여 인공지능에 적용하려는 시도가 가장 흥미로웠던 것 같다.그리고 비슷한 걸 배우겠지 라는 심정으로 논술로 세종대학교 신설학과인 인공지능학과를 써 보았다. 사실 알고있는거라고는 미분해서 극대 극소 구하는 것 밖에 모르지만 동아리를 통해 앞으로 전공하게 될 분야를 대략적으로라도 파악하고 싶어서 지원한다.

- 1학년 1학기 지원자님의 답변입니다. 이런 응답이 1학년 응답이라면 "잘 할 필요는 없다" 의 동아리 마인드에 따라서는 부족함 없는 답변입니다. 하지만, "써 보았다." "알고있는 거라고는," 의 표현은, "최소한의 노력" 의 의지도 보이지 않을 뿐더러 학과와 학교 자체를 쉽게 여기고 있는 것 같네요. 무슨 마음인지는 알겠지만, 저희는 좋은 점수를 드리기 어려웠습니다.
- *저희 원칙 "공개의 원칙", 지원자의 발전과 SAI 발전을 위한 "상호 발전의 원칙" 에 따라서 내용을 공개합니다.*

<br>

> 그리고 길게 보자면 저는 컴퓨터 분야에 흥미가 있는데 생시부를 오게 된 가장 큰 이유이기도 한 단백질 구조 예측에 관심이 많습니다. 재수를 하며, 이미 대학을 간 친구들과 가끔 연락을 하며 지냈는데 그 중 한 친구가 생명공학과 입니다. 그 친구도 코딩 쪽에 실력과 흥미가 있는 친구였는데 생명공학과를 간 이유를 물어보니 생물정보학이라는 분야에 꿈이 있어 갔다고 얘기를 해줬습니다. <중략> 생명 기술에 컴퓨터 기술을 섞은 BIT 분야를 친구의 조언으로 여러가지를 찾아보며 단백질 구조 예측이란 기술을 알게 되었습니다. 이 분야가 단순한 구조 예측이 아닌 신약 개발 등에도 적용이 가능한 엄청난 기술이라는 것을 알게 되었습니다. 이게 제가 동아리, 학부 수준에서 할 수 있는 수준은 아니겠지만 이런 쪽을 계속 공부하고, 신약 개발에 일조해 여러 질병들을 고치고 싶습니다.

- 1학년 1학기 지원자님의 답변입니다. 현실성과 상관없이, 이렇게 관심을 가지게 된 계기를 제시해 주세요.
- *저희 원칙 "공개의 원칙", 지원자의 발전과 SAI 발전을 위한 "상호 발전의 원칙" 에 따라서 내용을 공개합니다.*

<br>

> C와 Java를 모두 독학으로 공부하고, 전공이 스마트기기공학이라 인공지능쪽에 대해서도 공부를 하고 싶은데 이제 막 2학년으로 올라가는지라, 대학에서 해당 강의를 수강하는데에는 제한적인 부분이 있어, 별도로 공부를 해보고 싶어 관심을 갖게 되었습니다. 

- 2학년 1학기 지원자님의 답변입니다. 언어 둘을 독학으로 공부했다는 것은 굉장히 대단한 일이고, 혼자 하기 어려운 일임에도 해내셨다는 것이 대단합니다.
- 하지만, 인공지능과 관련해서는 왜 관심이 서칭과 독학으로 이어지지 않았는지가 조금 궁금합니다.
- *저희 원칙 "공개의 원칙", 지원자의 발전과 SAI 발전을 위한 "상호 발전의 원칙" 에 따라서 내용을 공개합니다.*

<br>

> 지능기전공학부에 들어오기 전까지는 인공지능에 솔직히 관심이 별로 없었지만 과에서 공부를 하다보니까 내가 설계하는대로 실현되는 부분에 흥미를 느껴서 동아리까지 지원하게 됐습니다.

- 3학년 1학기 지원자님의 답변입니다. 어떤 부분에서 흥미를 느꼈는지 저도 공감합니다.
- 하지만, 3학년 1학기 지원자이고, "설계한 대로 실현된다" 라는 것을 말할 수 있을 정도면, 구체적인 사례정도는 말씀해주실 수 있지 않을까? 하는 생각이 들어 아쉬웠습니다.
- *저희 원칙 "공개의 원칙", 지원자의 발전과 SAI 발전을 위한 "상호 발전의 원칙" 에 따라서 내용을 공개합니다.*

<br>

> 혼자서 공부하다보니 이렇게 공부하는 것이 맞는건지 불안할 때가 정말 많았습니다. 또 학교에 아는 사람도 없어서 물어볼 사람도 없습니다. 물론 혼자서 공부할 것을 찾아서 하는 게 맞지만 누군가의 조언이 필요할 때도 많고, 같이 공부할 사람이 있었으면 좋겠다고 생각할 때가 많습니다. 제가 SAI에서 공부하고 싶은 것은 체계적인 머신러닝,딥러닝 공부입니다. 그리고 함께 Kaggle 랭킹도 올려보고 싶습니다. 이번방학에 기본 파이썬 지식을 바탕으로 파이썬을 더 공부하고, Numpy와 Pandas를 공부해서 Kaggle의 몇 가지 데이터를 가지고 혼자 분석을 해봤습니다. 그리고 ML도 혼자 공부하기 시작했는데 아직은 초보 수준이지만 책을 따라해보면서 익히고 있습니다. 하지만 따라하는 것은 공부가 아니라는 것을 알고 있습니다. 체계적이지도 않아서 공부하면서도 완전히 이해하고 있는건지, 제 것이 되고 있는건지 의문이 들 때가 많습니다. 응용을 해봐야 완전히 제 것이 된다고 생각하는데 혼자서 응용을 해보기가 많이 어렵습니다. 그래서 동아리 팀원들과, 미리 공부해보신 선배님들을 통해서 어떻게 공부해야할지 조언을 듣고싶기도 합니다. 

- 2학년 1학기 지원자님의 답변입니다.
- 시행착오가 담겨 있지 않나요?
- *저희 원칙 "공개의 원칙", 지원자의 발전과 SAI 발전을 위한 "상호 발전의 원칙" 에 따라서 내용을 공개합니다.*

<br>

### 그 외, 미래의 지원자분들과, 이번 지원자분들이 알아 주셨으면 하는 피드백입니다.

<br>

> 우선 파이썬 머신러닝 완벽 가이드라는 책의 예제를 혼자힘으로 다 풀어보고 싶다.후에 더 다양한 자료와 문제들을 풀어감으로써 차근차근 내가 연구할(정착할) 분야를 선택하고 싶다.

> 자료구조 알고리즘 스터디 진행해주세요!

- 위에서 말씀드렸듯이, SAI 는 교육기관이 아닙니다. 인공지능에 대해서 잘 할 필요는 없지만, 최소한 언어적인 내용은 스스로 해 주셔야 운영진이 다른 부분에 집중할 수 있습니다.
- 정말 안타까운 것 중 하나가, 전공자 2~3학년이 될때까지 심지어는 4학년이 될때까지 언어에 집착하시는 분들이 계십니다.
- 나 자신에게 "뭐 공부할까?" 를 물었을 때 C/python/java 와 같은 언어가 그 대답으로 나온다면, 아직 충분한 고민이 선행되지 않았다고 생각합니다. 언어는 도구일 뿐입니다.
- 비슷한 맥락에서 학교 교과목에 매달리는 분들이 계시는데, 기본이 나쁘다는 것이 절대 아니라 그건 스스로 하시거나 다른 스터디를 찾아보시는 것이 더 효율적일 것 같다는 생각이 들었습니다.
- 저희가 어느 의견도 괜찮다고 폼을 받았고 위 내용을 작성해 주신 것도 감사하지만, 이를 명시하는 것은 폼에 작성해주신 내용들이 시행되지 않았을 때에 대한 오해를 없애기 위해 미리 설명드립니다.
- *저희 원칙 "공개의 원칙", 지원자의 발전과SAI 발전을 위한 "상호 발전의 원칙" 에 따라서 내용을 공개합니다.*

<br>

> 아직 저의 실력이 좀 부족하기때문에 우선은 인공지능에 대해 더 배우는 것을 원하고 조금 숙련이 되었을 때 해커톤과 같은 교내 대회에 나가보고 싶습니다!

- 저도 항상 부족합니다. 하지만 제가 조금 더 믿는 부류의 자기소개서는 - 부족하더라도 대회에 나가보고, 나가서 부족함을 다시 깨닫고, 뭐가 필요한지를 알아서 공부하는 사람의 자기소개서입니다. 항상 부족하다고만 생각하면, 그것은 5년을 공부해도 10년을 공부해도 항상 부족함을 느끼게 된다고 생각해요. 적어도 그 도전 자체만으로도 그 사람의 관심이 증명되기 때문입니다.

<br>

### 재미있는 응답 몇 개

- Q. SAI 동아리 라이프! 동아리에 대해서 상상하는 것이 있다면? 무엇인가요? 예를 들면, 강화학습으로 만든 AI 로 서로 오목 둬보기 라던가. 스터디 끝나고 함께 치킨 먹으러 가서 프로젝트 이야기하기 라던가.. 어때요?
- A. 제 첫사랑 이루다가 없어졌어요ㅠㅠ 한 3단 너프된 버전이라도 좋으니까 같이 여자친구 만들어보시지 않을래요? -> *저도 딱 고3 끝나고 비슷한 경험이 있습니다. 유감이지만 지금은 있습니다.*
- A. 인공지능 공부를 하다가 일상이 인공지능으로 물들어 잘 때도 인공지능 꿈을 꿀만큼 동아리원분들과 열심히 공부하고싶습니다. 사실 공부도 중요하지만 좋은 인맥을 쌓는것도 동아리의 큰 장점이라고 생각하는데, 공부 끝나고 같이 밥을 먹는다든가, 가끔씩 놀러 가는 것도 좋다고 생각합니다 -> *WOW*

<br><br>

# 지원서 예시

<br>

- 공개 수락자에 한하여 게시
- 매우 긍정 예시, 커트라인 예시

<br>

### 공개목적

- 공정성, 투명성
- 명확한 기준 제시 및 서로 관심분야 파악하기

<br>
<hr>

## 매우 긍정 예시

<br>

### 매우 긍정 Example 1

| 동아리 지속성, 성실성 기대 (동아리 적합성, 열정) | 타당성 + 개연성 + 실력 (개연성 있는 성장, 그동안 걸어온 길) | 다른단과대 | 학년 |
|:-------------------------------:|:-------------------------------------------:|:----------:|:----:|
| 4점, 4점, 4점, 4점 | 3점, 3점, 3점, 3점 | 경영+소프트 | 4학년 | - |
| 16 | 12 | 2 | 0 | 30 |

<br>

### 프로그래밍/배경 지식

- python 에서 자료형 int, float, string 의 차이를 앎
- python 에서 리스트(list) 에 원소를 추가하거나 삭제할 줄 앎.
- python 에서 import 를 사용할 줄 앎.
- python 에서 import 를 할 경우 컴퓨터의 어떤 경로에서 모듈을 찾아오는지 앎.
- (언어무관) 객체지향 문법과 class 의 개념에 대해서 알고 있음.
- (언어무관) 환경변수 가 무엇인지 앎.
- 대학교에 처음 들어와서 소프트웨어를 공부했습니다.
- 예체계열이기 때문에 수능수학 및 대학수학에 익숙하지 않습니다.
- 인공지능 기술 (머신러닝 전반) 에 있어 관련 소규모 프로젝트를 해 본 적이 있다.
- Python 으로 인공지능을 구현하는 프레임워크의 이름들을 3가지 이상 알고 있다.
- Neural Network 에서 초기화 문제가 왜 중요한지, activation function 과 어떤 관계가 있는지 어느정도 감을 잡고 있다.
- 데이콘과 캐글 등이 무엇을 하는 플랫폼인지 알고 있다.
- 오프라인 컨퍼런스나 개발 관련 행사, 학회, 인공지능 관련 행사들에 2회이상 참석해 보았다.
- 교내외 인공지능 관련 캠프 (1달과정 이상) 등을 이수한 적 있다.
- 인공지능 분야의 (한글, 영어) 논문을 1개 이상 읽어 보았다.

<br>

### 단과대학, 학과, 학년

- 경영대학, 소프트웨어융합대학
- 주전공 경영학과 복수전공 소프트웨어학과
- 4학년 진행중, 수료

<br>

### 인공지능 자체에 관심가지게 된 계기가 무엇인가요?

처음에는 인공지능이 그저 미래가 가장 밝은 기술이라고 생각되어 관심을 가지기 시작했습니다. 하지만 정확히 어떻게 활용할 수 있고 어떤 방식으로 동작하는 지에 대한 것은 아주 무지했습니다. 이후에 휴학을 하고 어떤 것을 공부해야할까 고민하던 중 대외활동으로 AI school이라는 곳을 알게되었습니다. 그곳에서는 인공지능의 기초 수업을 해주고 각자 팀을 꾸려서 프로젝트를 진행했습니다. 그렇게 처음으로 인공지능을 접하게 되었습니다. 저희 팀은 프로젝트로 빅콘테스트 퓨쳐스리그 대회에 나갔습니다. 처음 프로젝트를 진행하면서 정말 힘들고 때로는 밤도 새면서 고생을 많이 했지만 하면 할수록 결과물이 나오는 것을 보면 희열을 느꼈습니다. 그래서 인공지능 분야에 좀 더 깊이 공부하고 활용할 수 있는 개발자가 되고싶다고 느꼈습니다. 그리고 운이 좋게도 대회에서 장려상을 수상하게 되었고, 이로써 더욱더 인공지능에 관심을 가지고 이 길로 나아갈 수 있다는 확신이 생겼습니다.

<br>

### 동아리에 들어오면 하고 싶은 프로젝트 또는 내가 인공지능을 적용해서 해결해 보고 싶었던 문제가 있나요? 다른 사람들이 흔히 생각할 문제 말고, 내가 관심가지고 살던 분야에서 인공지능을 적용하고 싶었던 적이 있나요?

제가 생각했던 프로젝트는 피부병 자가검진입니다. 이 프로젝트는 사람들이 발진이나 물집과 같이 뭔가 피부에 이상이 있을 때 병원에 가기 전 네이버 지식인에 사진을 올리고 물어보는 것을 보고 떠올린 아이디어입니다.
 피부질환은 원인에 따라 같은 홍반이라도 위치나 퍼져있는 범위가 달라 이를 오브젝트 디텍션으로 구분해낼 수 있을 것이라고 생각했습니다. 병원에서 진단을 받기 전 간단한 간이 검사를 통해서 막연한 불안감을 진정시켜주고 간단히 할 수 있는 대처 방안과 해서는 안되는 민간요법을 알려주는 간단한 앱을 만들면 좋을 것 같습니다.
  위 프로젝트를 학습시키는 방법은 다음과 같습니다. 레이블은 대상포진이나 화상, 습진, 피부염 등 병명으로 둡니다. 이에 대한 피쳐는 사진, 부위, 증상에 대한 내용을 담은 피쳐를 사용할 것입니다. 데이터를 수집하는 데에는 두 가지 방법을 생각했습니다. 첫째는 구글에서 피부질환에 대한 이미지를 크롤링하는 것입니다. 이 때, 사진의 병에 대한 병명은 정확한 판단을 위해 가능하다면 전문의의 소견을 얻어 진단하고 레이블로 설정합니다. 둘째는 사람들이 지식인에 올린 사진과 그에 대해 전문의의 답변을 토대로 크롤링하는 것입니다. 위 방법들을 통해서 데이터를 수집하고 어그멘테이션을 통해서 데이터를 증가시키며 밝기나 크기가 다양한 상황에서도 예측할 수 있도록 만듭니다.
  
<br>
  
### 동아리에 들어오면 배우고 싶은 것이 무엇인가요? 어떤 공부를 함께하고 싶나요?
 
 저는 파이썬을 통해서 모델을 구성하는 방법 정도만 아는 수준입니다. 그래서 이 동아리에서의 스터디를 통해 수학적인 부족함을 채우고 싶습니다. 복수전공이면서 원래 주전공이 문과여서 수학적인 면에서 많이 부족하다고 느끼고 있습니다. 그래서 함께 공부하면서 혼자 하는 것보다는 보다 어렵지 않게 다가가고 싶습니다.
 그리고 다른 사람들은 어떻게 공부하고 어떤 프로젝트를 생각했으며 어떤 방식으로 진행하는 지 많은 사람들을 접하며 보고싶습니다. 어떻게 공부하고 이를 프로그래밍에 적용하는 지 다른 사람들을 보며 저도 배우고 싶습니다.
 마지막으로 취업하신 분이 있다면 어떻게 준비했고 어떤 걸 더 준비해야하는지와 같은 현실 조언과 대학원 진학을 고민하고 있는데 이에 대한 상담을 해주실 수 있다면 더할 나위 없이 감사할 것 같습니다.
 
<br>

### SAI 동아리 라이프! 동아리에 대해서 상상하는 것이 있다면? 무엇인가요? 예를 들면, 강화학습으로 만든 AI 로 서로 오목 둬보기 라던가. 스터디 끝나고 함께 치킨 먹으러 가서 프로젝트 이야기하기 라던가.. 어때요?

 같은 분야를 생각하는 사람들끼리만할 수 있는 이야기를 할 수 있는 분위기!가 기대됩니다. 복전생은 생각보다 같은 이야기를 할 친구가 없는데 그런 사람들이 생긴다면 너무 행복할 것 같습니다.
 팀별로 같은 데이터를 쥐어주고 예측 정확도 높이기 대회를 해도 재밌을 것 같습니다!
 
 <br>
 
 ### 관심 분야
 
- 자연어 처리 및 신호처리 전반
- 컴퓨터 비전 전반
- 통계 기반의 머신러닝 및 데이터 분석

<br><br>
<hr>

### 매우 긍정 Example 2

| 동아리 지속성, 성실성 기대 (동아리 적합성, 열정) | 타당성 + 개연성 + 실력 (개연성 있는 성장, 그동안 걸어온 길) | 다른단과대 | 학년 |
|:-------------------------------:|:-------------------------------------------:|:----------:|:----:|
| 4점, 4점, 4점, 4점 | 3점, 3점, 3점, 3점 | 소프트 | 4학년 | - |
| 16 | 12 | 0 | 2 | 30 |

<br>

### 프로그래밍/배경 지식

- python 에서 자료형 int, float, string 의 차이를 앎
- python 에서 리스트(list) 에 원소를 추가하거나 삭제할 줄 앎.
- python 에서 import 를 사용할 줄 앎.
- python 의 자료형들은 결국 class 들이라는 것을 앎.
- (언어무관) 객체지향 문법과 class 의 개념에 대해서 알고 있음.
- 대학교에 처음 들어와서 소프트웨어를 공부했습니다.
- 수능공부를 했기 때문에 수능수학 및 대학수학에 익숙합니다.
- 인공지능 기술 (머신러닝 전반) 에 있어 관련 소규모 프로젝트를 해 본 적이 있다.

<br>

### 단과대학, 학과, 학년

- 응용통계학과
- 컴퓨터공학과 (복수)
- 4학년 진행중, 수료

<br>

### 인공지능 자체에 관심가지게 된 계기가 무엇인가요?

평소 데이터 분석에만 관심을 두던 저는 인공지능 지식은 0에 수렴하던 상태였습니다.
그러다 드라마 ‘스타트업’을 보며  ’물체를 인식하는 기능‘의 인공지능을 활용하여  ’시각장애인을 위한 앱’을 개발하는 것을 보고 인공지능이 현실 속에서 저렇게 유용하게 사용될 수 있음을 처음으로 느꼈고 이를 계기로 인공지능에 호기심이 생겼습니다.
그렇게 호기심이 가득할 때쯤 도전하는 것을 좋아하는 저는 세종대학교 인공지능 해커톤에 참여해봤습니다. 인공지능에 지식이 없던 저와 팀원들은 어떻게든 결과물을 만들어내기 위해 인공지능에 관해 공부하기 시작했습니다. 그러다 구글에서 발표한 ‘Teachable Machine 2.0’을 알게 됐고 어렵지 않은 사용법으로 바로 적용해보았습니다. 그리고 이를 이용하여 빨간 국물과 맑은 국물을 구별해보는데, 사용법도 간단하고 결과도 잘 나오는 것이 너무 신기했습니다.
 비록 해커톤 수상은 남 일이 되었지만, 이것이 제 인공지능 세계로의 첫걸음이었습니다. 사진을 이용해서 기계를 학습시킨 뒤, 학습시킨 바탕으로 새로운 사진을 판단하여 분류하는 것이 너무 흥미롭고 재밌었습니다. 멀게만 느껴지던 인공지능이 이렇게 가까이 와있다는 것에 더욱 흥미가 생겼고 이에 대해 더 깊숙이 공부하고 싶었습니다.
하지만 해커톤이 끝나고 나서 개인적인 흥미로 지속적인 공부를 이어가기는 쉽지 않음을 느꼈습니다. 그래서 함께 공부하고 성장할 기회를 찾던 중 SAI 동아리를 발견했습니다. 이 동아리에서 저와 같이 인공지능에 흥미를 느낀 사람들을 만나고 싶습니다. 꼭 함께  그리고 재밌게 성장하고 싶습니다.  

<br>

### 동아리에 들어오면 하고 싶은 프로젝트 또는 내가 인공지능을 적용해서 해결해 보고 싶었던 문제가 있나요? 다른 사람들이 흔히 생각할 문제 말고, 내가 관심가지고 살던 분야에서 인공지능을 적용하고 싶었던 적이 있나요?

제가 진행하고 싶은 프로젝트중 하나는 하나의 웹사이트에서 여러가지 GAN모델(GAN,DCGAN,CYCLEGAN,STYLEGAN)이 학습되는 진행을 보여주는 사이트를 만드는것이 가장 큰 목표입니다. 지금까지 다른 사람들이 호스팅하는 웹사이트에서는 주로 GAN이 무엇이고 어느 어플리케이션이 있고 등이였지만, 저는 누구나 데이터를 정해진 방식에 따라 넣고 원하는 모델을 선택하면 학습이 진행되면서 모델을 훈련하고 중간중간 모델의 결과물들을 사람들이 잘 볼수 있도록 시각화를 잘 정제하여 보여주는 중앙 플랫폼을 만드는 것을 최종 목적으로 잡고 있습니다. 또한 진행하고 싶은 다른 프로젝트는 강화학습을 이용한 학습 훈련과정을 보여주는 웹사이트를 만드는 것입니다. 이 또한 첫번째로 말씀드린 프로젝트와 같이 한 홈페이지에서 여러가지 보상함수들을 사용자가 선택할수 있게 해주고 그에 따른 훈련의 진행에 따른 결과물들을 시각화하여 보여주는 프로젝트를 진행하고 싶습니다. 

<br>

### 동아리에 들어오면 하고 싶은 프로젝트 또는 내가 인공지능을 적용해서 해결해 보고 싶었던 문제가 있나요? 다른 사람들이 흔히 생각할 문제 말고, 내가 관심가지고 살던 분야에서 인공지능을 적용하고 싶었던 적이 있나요? (최대 1000자)
1-1)학원에서 일을 하다 보니 모르는 문제가 있는데 질문을 하기 부끄러워하는 내성적인 아이를 만났을 때 가장 안타까웠습니다. 어려운 문제 앞에서 시간을 오래 끌고 있을 때, 과연 이 아이가 고민하느라 오래 걸리는 건지, 아니면 어려움을 말하지 못해서 주저하고 있는 것인지 구별해내고 싶었습니다. 그러다 떠올린 아이디어는 어려운 문제를 만났을 때 당황스러워하는 학생의 표정, 오랫동안 가만히 있는 연필을 잡은 손, 움직이지 않고 한곳에 오래 머무르는 동공을 발견하여 아이가 어려움을 말하지 못함을 인지할 수 있는 시스템입니다.또한 이를 발견했을 때 선생님에게만 알림이 뜨거나, 그 학생에게만 문제를 수월하게 해결할 수 있는 힌트를 조용히 제공하는 서비스를 프로젝트로 만들고 싶습니다. 
그리고 아이의 표정을 읽어 졸음을 인지하여 스스로 잠을 깰 수 있는 알림도 만들고 싶습니다. 문제를 풀다가 조는 아이들은 자신이 졸고 있음을 잘 인지하지 못하기 때문에 잠을 내쫓을 타이밍을 잡기 어려워하기 때문입니다.

1-2) 제가 말한 프로젝트는 아이들을 대상으로 생각해낸 아이디어지만, 이를 대학생에게도 적용해보면 좋을 것 같습니다. 교수님께 질문하기 부끄러운 학생들을 위해, 학생들의 표정을 읽어 더 자세한 설명의 필요성을 느낄 수 있게 알림을 주는 인공지능을 만들면 교수님께서도 수업을 수월하게 진행할 수 있을 것입니다.
또 혼자 공부를 할 때 졸음을 인지시켜주고 깨워주는 프로젝트도 만들면 좋을 것 같습니다.

2) 최근에 마스크로 인해 피부에 여드름이나 뾰루지가 증가하는 것을 보고 피부 관심도가 높아졌습니다. 인공지능으로 내 피부에 난 것이 어떤 여드름인지(좁쌀, 화농성, 뾰루지, 접촉성피부염 등등) 종류를 파악하고, 어떻게 케어하는 것이 좋은지 방법을 알려주는 프로젝트도 진행해보고 싶습니다. 피부과까지 가지 않고도 자신의 피부 염증을 파악할 수 있다면 피부에 대한 걱정과 고민을 줄일 수 있고, 피부과에서도 과잉진료나, 필요 없는 시술을 피하는 데 큰 도움이 될 것입니다.

<br>
### 동아리에 들어오면 배우고 싶은 것이 무엇인가요? 어떤 공부를 함께하고 싶나요? (최대 1000자)
인공지능에 대한 관심은 매우 크나, 지식과 경험이 부족한 저로서는 동아리에서 하는 모든 것을 배움으로 받아들이고 싶습니다. 특히 저와 같이 인공지능에 관심을 가지고 있는 사람들은 어떤 아이디어를 품고 살아가는지, 어떻게 공부하고 어떻게 인공지능에 대해 다가가는지 배우고 싶습니다. 또 열심히 따라가며 인공지능을 실생활에 적용시킬 수 있는 경험을 얻고 싶습니다. 프로젝트를 기획부터 결과 발표까지의 과정을 겪어보지 못한 제게는 프로젝트라는 단어만 들어도 설렙니다. 같이 해낸 프로젝트로 좋은 결과를 만들어내고, 팀원들과 함께 성장하는 기쁨, 그리고 그 뿌듯함을 느끼고 배우고 싶습니다. 
또 응용통계학과를 복수전공을 하다 보니 빅데이터 분야에도 큰 관심이 있습니다. 데이터 분석에 관심을 두는 사람들을 모아 ADSP 또는 ADP 취득을 목표로 함께 공부하는 소모임도 만들어보고 싶습니다. 꼭 위와 같은 자격증이 아니더라도 같이 데이터 분석 프로젝트도 해보고 빅데이터 공모전도 준비해보고 싶습니다. 제가 알고있는 R이나 Python 지식을 나누며 좋은 결과물을 만들어보고 싶습니다.

<br>

### SAI 동아리 라이프! 동아리에 대해서 상상하는 것이 있다면? 무엇인가요? 예를 들면, 강화학습으로 만든 AI 로 서로 오목 둬보기 라던가. 스터디 끝나고 함께 치킨 먹으러 가서 프로젝트 이야기하기 라던가.. 어때요?
너무 좋습니다.. 이제 4학년이 된 저는 코로나 때문에 학교생활을 충분히 즐기지 못했습니다. 이렇게 캠퍼스 라이프를 즐겨보지도 못하고 졸업을 하는 것은 너무 억울하기에 SAI 팀원들과 정말 많은 것들을 해보고 싶습니다.
스터디 끝나고 어린이대공원 정자에서 같이 치킨과 피자를 먹으며 제 폴라로이드로 사진을 남기고 싶습니다.
그리고 제가 예전에 아르바이트했던 롯데타워 전망대에서 같이 만든 인공지능을 활용해 재밌는 실험도 해보고 싶습니다. (어떤 실험을 할지 구체적인 아이디어는 생각해보지 않았지만, 사진으로 무슨 대교인지 파악해주는 앱 만들기, 돌발 행동 인지하고 경보음 울리는 인공지능 만들기 등등.. )
또한 롯데월드에 입장할 인원수를 예측해주는 인공지능으로 제일 사람이 적은 날을 골라 다 같이 롯데월드 가서 놀이기구를 타며 적은 인원수를 몸소 체험하고 즐기고 싶습니다.

그리고 무엇보다 제일 해보고 싶은 것은 동아리 팀원들과 가벼운 분위기에서 깊은 듯 깊지 않은 이야기를 나눠보고 싶습니다. 다들 어떤 가치관으로 어떤 인생을 살아왔고 앞으로는 어떻게 살아가고 싶은지 바람 선선한 날에 맥주 한 캔 마시며 소소하지만 소중한 추억을 쌓아보고 싶습니다.

<br>

### 관심 분야
 
- 통계 기반의 머신러닝 및 데이터 분석

<br><br><br>
<hr>

## 커트라인 예시

<br>

### 커트라인 Example 1

| 동아리 지속성, 성실성 기대 (동아리 적합성, 열정) | 타당성 + 개연성 + 실력 (개연성 있는 성장, 그동안 걸어온 길) | 다른단과대 | 학년 |
|:-------------------------------:|:-------------------------------------------:|:----------:|:----:|
| 2점, 3점, 2점, 3점 | 2점, 1점, 1점, 2점 | 소프트 | 3학년 | - |
| 10 | 6 | 0 | 1 | 17 |

<br>

### 프로그래밍/배경 지식

- python list 변수가 값이 아니라 주소라는 것을 앎
- 대학교에 처음 들어와서 소프트웨어를 공부했습니다.
- 모두를 위한 딥러닝이라는 강좌를 접한 적은 있지만 많이 수강하지는 않았다.

<br>

### 단과대학, 학과, 학년

- 소프트웨어융합대학
- 데이터사이언스학과
- 2학년 진행중, 수료

<br>

### 인공지능 자체에 관심가지게 된 계기가 무엇인가요?

과가 데이터분석에관한 학과이기도하고 전공수업을 들으며 공부하다보니 수업만으로는 아직 이 분야에대해 제가 부족하고 지식이 채워지지않는것같아서 이 동아리를 통해 인공지능에대해 더 깊이 알고싶다고 생각하게 되었습니다

<br>

### 동아리에 들어오면 하고 싶은 프로젝트 또는 내가 인공지능을 적용해서 해결해 보고 싶었던 문제가 있나요? 다른 사람들이 흔히 생각할 문제 말고, 내가 관심가지고 살던 분야에서 인공지능을 적용하고 싶었던 적이 있나요?

아직 특별히 해보고싶은 프로젝트는 없습니다. 하지만 좀더 공부를 해보고 다양한 적용사례들을 접하다보면 자연스레 생길것이라고 생각합니다

<br>

### 동아리에 들어오면 배우고 싶은 것이 무엇인가요? 어떤 공부를 함께하고 싶나요?

데이터분석과 인공지능에대해 기초적인부분부터 깊이있게 제대로 배우고싶습니다 아직 어디서부터 어떻게 시작해야할지 막막한데 이 동아리를 통해서 성장하고싶습니다! 그렇게해서 해커톤과같은 대회도 함께 도전하고 프로젝트도 진행할수있을정도로 저의 역량을 키우고싶습니다. 이를 통해 궁극적으로는 제가 어느분야로 나아가야할지 방향성에 대해서도 생각해보고싶습니다.

<br>

### SAI 동아리 라이프! 동아리에 대해서 상상하는 것이 있다면? 무엇인가요? 예를 들면, 강화학습으로 만든 AI 로 서로 오목 둬보기 라던가. 스터디 끝나고 함께 치킨 먹으러 가서 프로젝트 이야기하기 라던가.. 어때요?

생각만해도 설렙니다!!🥰 벌써3학년인데 아직 제대로된 동아리활동도 해보지않아서 이번기회에 이 동아리를 통해서 사람도 얻고 지식도 넓히며 대학생활을 알차게 보내고싶습니다 

<br>

### 관심 분야
 
- 통계 기반의 머신러닝 및 데이터 분석

<br><br>

<hr>

### 커트라인 Example 2

| 동아리 지속성, 성실성 기대 (동아리 적합성, 열정) | 타당성 + 개연성 + 실력 (개연성 있는 성장, 그동안 걸어온 길) | 다른단과대 | 학년 |
|:-------------------------------:|:-------------------------------------------:|:----------:|:----:|
| 1점, 2점, 1점, 2점 | 3점, 2점, 1점, 2점 | 공과+소프트 | 4학년 | - |
| 6 | 8 | 2 | 0 | 16 |

<br>

### 프로그래밍/배경 지식

- python 에서 자료형 int, float, string 의 차이를 앎
- python 에서 리스트(list) 에 원소를 추가하거나 삭제할 줄 앎.
- python 에서 import 를 사용할 줄 앎.
- python 에서 import 를 할 경우 컴퓨터의 어떤 경로에서 모듈을 찾아오는지 앎.
- python list 변수가 값이 아니라 주소라는 것을 앎
- python 의 자료형들은 결국 class 들이라는 것을 앎.
- (언어무관) 객체지향 문법과 class 의 개념에 대해서 알고 있음.
- (언어무관) 공식 document 를 읽고 코드를 작성해 본 적 있음.
- (언어무관) 환경변수 가 무엇인지 앎.
- (언어무관) Cuda, hadoop 등을 잘 알고 있음.
- 모두를 위한 딥러닝이라는 강좌를 모두 수강했다.
- 교내 인공지능 수업을 이수했고, B0 이상의 학점을 받았다.
- 인공지능 기술 (머신러닝 전반) 에 있어 관련 소규모 프로젝트를 해 본 적이 있다.
- CS231n 이라는 강좌를 접한 적은 있지만 많이 수강하지는 않았다.
- Python 으로 인공지능을 구현하는 프레임워크의 이름들을 3가지 이상 알고 있다.
- Neural Network 역전파 알고리즘을 수학적 (chain rule) 으로 보일 수 있다.
- Neural Network 에서 초기화 문제가 왜 중요한지, activation function 과 어떤 관계가 있는지 어느정도 감을 잡고 있다.
- 인공지능 관련으로 연구하시는 교수님의 LAB의 학부 연구생이고, 6개월 이상 지났다.
- 데이콘과 캐글 등이 무엇을 하는 플랫폼인지 알고 있다.
- 오프라인 컨퍼런스나 개발 관련 행사, 학회, 인공지능 관련 행사들에 2회이상 참석해 보았다.
- 인공지능 분야의 (한글, 영어) 논문을 1개 이상 읽어 보았다.

<br>

### 단과대학, 학과, 학년

- 공과대학, 소프트웨어융합대학
- 주전공 항공우주학과, 컴퓨터공학과
- 4학년 진행중, 수료

<br>

### 인공지능 자체에 관심가지게 된 계기가 무엇인가요?

공군SW지원소에서 군 복무를 하면서 다양한 언어를 경험할 수 있었고, 복학후에는 교내 연구실에서의 학부연구생활동과 컴공복전을 통한 인공지능 수업을 통해서 인공지능에 관심을 갖게됐습니다.

<br>

### 동아리에 들어오면 하고 싶은 프로젝트 또는 내가 인공지능을 적용해서 해결해 보고 싶었던 문제가 있나요? 다른 사람들이 흔히 생각할 문제 말고, 내가 관심가지고 살던 분야에서 인공지능을 적용하고 싶었던 적이 있나요?

현재는 컴퓨터비전기반으로 프로젝트를 수행했었지만 이외에도 다양한 분야를 겅험하고 싶습니다.

<br>

### 동아리에 들어오면 배우고 싶은 것이 무엇인가요? 어떤 공부를 함께하고 싶나요?

강화학습기반의 다양한 프로젝트

<br>

### SAI 동아리 라이프! 동아리에 대해서 상상하는 것이 있다면? 무엇인가요? 예를 들면, 강화학습으로 만든 AI 로 서로 오목 둬보기 라던가. 스터디 끝나고 함께 치킨 먹으러 가서 프로젝트 이야기하기 라던가.. 어때요?

관심사가 비슷한 학우들간의 네트워크 형성

<br>

### 관심 분야

- 컴퓨터 비전 전반
- 강화 학습 및 게임 AI

<br><br><br>
<hr>

## 공백 예시

<br>

### 프로그래밍/배경 지식

<br>

### 단과대학, 학과, 학년

<br>

### 인공지능 자체에 관심가지게 된 계기가 무엇인가요?

<br>

### 동아리에 들어오면 하고 싶은 프로젝트 또는 내가 인공지능을 적용해서 해결해 보고 싶었던 문제가 있나요? 다른 사람들이 흔히 생각할 문제 말고, 내가 관심가지고 살던 분야에서 인공지능을 적용하고 싶었던 적이 있나요?

<br>

### 동아리에 들어오면 배우고 싶은 것이 무엇인가요? 어떤 공부를 함께하고 싶나요?

<br>

### SAI 동아리 라이프! 동아리에 대해서 상상하는 것이 있다면? 무엇인가요? 예를 들면, 강화학습으로 만든 AI 로 서로 오목 둬보기 라던가. 스터디 끝나고 함께 치킨 먹으러 가서 프로젝트 이야기하기 라던가.. 어때요?

<br>

### 관심 분야
 
<br><br>
 
